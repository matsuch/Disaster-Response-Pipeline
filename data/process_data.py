...
def clean_data(df):
    """
     Function:
      ...
      """
    #split categories into categorie columns
    categories = df['categories'].str.split(';',expand=True)
    
    #identify the first row values and replace any value after the character '-' with ''
    rows = categories.iloc[0]
    rows = rows.replace('\-.*$', '', regex=True)
    
    #replace the column name based on the row value after the replace operation
    category_colnames = rows.tolist()
    categories.columns = category_colnames
    
    #Convert category values to just numbers 0 or 1 based on the string last value
    ...
    #replace the related values to 1
...
def save_data(df, database_filename):
    ...
    df.to_sql('disaster_massages_table', engine, index=False, if_exists='replace')
